...
class ReceiverMessageItemWidget extends StatefulWidget{
  ...
  const ReceiverMessageItemWidget(
      {super.key, required this.message, required this.size, required this.roomID,required this.sender, this.isGroupChat = false});

  @override
  State<ReceiverMessageItemWidget> createState() => _ReceiverMessageItemWidgetState();
}
            ...
            children: [
              if(widget.isGroupChat)
                Padding(
                  ...
                  child: Text(widget.sender.userName, style: AppTextStyles.regularTextStyle,),
                ),
              CustomPaint(
                ...
                child: Container(
                  // margin: const EdgeInsets.symmetric(horizontal: 25, vertical: 10),
                    ...
                    child: LinkifyTextMessage(messageText: widget.message.messageText, isReceiver: true,)
  ...
  Future<void> _isReadByRecipient() async {
    ...
    if(!widget.message.isReadByReceiver){
      ...
      if(widget.isGroupChat){
        ...
        await ChatService.markAsRead(messageID: widget.message.messageID, roomID: widget.roomID);
      }
    }
  }
}