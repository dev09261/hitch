...
import 'package:hitch/src/services/promo_service.dart';
...
class _SubscriptionPaywallState extends State<SubscriptionPaywall> {
                        ...
                        children: [
                                ...
                                TextSpan(
                                  recognizer: TapGestureRecognizer()
                                    ..onTap = () => InAppPurchaseConfig
                                        .onRestorePurchaseTap(context: context),
                          ...
                          RichText(
                              ...
                              text: TextSpan(children: [
                                const TextSpan(
                                  text:
                                      "By purchasing Hitch subscription, you accept our ",
                                  style: TextStyle(
                                      fontSize: 14,
                                      color: AppColors.greyTextColor,
                                      fontFamily: 'Inter'),
                                ...
                                TextSpan(
                                  recognizer: TapGestureRecognizer()
                                    ..onTap = () =>
                                        Utils.launchAppUrl(url: termsOfUseUrl),
              ...
              child: isMonthly
                  ? Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${_getMonthlyPackage?.storeProduct.priceString}/m (cancel anytime)',
                          style: const TextStyle(
        ...
        child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${_getPromoPackage?.storeProduct.priceString}/m (cancel anytime)',
                  style: const TextStyle(
                      fontSize: 17, fontStyle: FontStyle.italic),
                ),
                Row(
                  children: [
                    ...
                    PrimaryBtn(
                      ...
                      onTap: () async {
                        if (_promoCtrl.text != '') {
                          String code = _promoCtrl.text;
                          _promoCtrl.text = "";
                          setState(() {
                            _isPromoLoading = true;
                          });
                          if (await PromoService().checkPromoCode(code, context)) {
                            _onPurchasePromoPackage(code);
                          }
                          setState(() {
  ...
  Package? get _getMonthlyPackage {
    ...
    return _availablePackages
        .firstWhere((package) => package?.packageType == PackageType.monthly);
  }
  ...
  Package? get _getAnnualPackage {
    ...
    return _availablePackages
        .firstWhere((package) => package?.packageType == PackageType.annual);
  }

  Package? get _getPromoPackage {
    if (_availablePackages.isEmpty) {
      return null;
    }
    Package? _promoPackage;
    for (var item in _availablePackages) {
      if (item!.storeProduct.identifier.toLowerCase().contains("m2")) {
        _promoPackage = item;
      }
    }
    return _promoPackage;
  }
  ...
  void _initPackages() async {
    ...
    try {
      ...
      List<Package> subscriptionPackages =
          await InAppPurchaseConfig.getSubscriptionPackages();
  ...
  Future<void> _onPurchasePromoPackage(String code) async {
    FocusManager.instance.primaryFocus?.unfocus();
    setState(() => _subscriptionLoading = true);
    Package? package = _getPromoPackage;
    try {
      CustomerInfo customerInfo = await Purchases.purchasePackage(package!);
      if (customerInfo.activeSubscriptions.isNotEmpty) {
        //Update Subscription Provider
        _updateProvider(customerInfo);
        PromoService().updateUsePromo(code);
      }
      if (widget.popupUpOnSubscription) {
        _onPopup();
      }
