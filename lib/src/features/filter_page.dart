...
class _FilterPageState extends State<FilterPage> {
    ...
    return isSubscribed ? PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title:  const Text("Filter", style: AppTextStyles.pageHeadingStyle,),
          centerTitle: true,
          scrolledUnderElevation: 0,
          backgroundColor: _changesLoading ? Colors.black54 :  Colors.white,
        ),
        body: SafeArea(
          child: Stack(
            children: [
              SingleChildScrollView(
                ...
                child: loadingUserInfo
                  ...
                  children: [
                    const Text("Distance from current location", style: TextStyle(fontSize: 21, fontWeight: FontWeight.w500, color: AppColors.primaryColor, fontFamily: 'Inter'),),
                    const SizedBox(height: 20,),
                    HitchSliderWidget(onChange: onSliderChange, distanceFromCurrentLocation: _distanceFromCurrentLocation,),

                    const SizedBox(height: 40,),
                    const Text("Type", style: TextStyle(fontSize: 21, fontWeight: FontWeight.w500, color: AppColors.primaryDarkColor),),
                    const SizedBox(height: 10,),
                    InkWell(
                      onTap: ()=> setState(()=> _filterTypePickleBall = !_filterTypePickleBall),
                      child: Padding(
                        padding: const EdgeInsets.symmetric(vertical: 10.0),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            const Text(playerTypePickleBallValue, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w400),),
                    ...
                    const Divider(),
                    InkWell(
                      onTap: ()=> setState(()=> _filterTypeTennisBall = !_filterTypeTennisBall),
                      child: Padding(
                        padding: const EdgeInsets.symmetric(vertical: 10.0),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            const Text(playerTypeTennisValue, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w400),),
                    ...
                    if(_filterTypeTennisBall)
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text("Tennis (UTR)", style: TextStyle(fontSize: 21, fontWeight: FontWeight.w500, color: AppColors.primaryDarkColor),),
                          const SizedBox(height: 10,),
                          Column(
                            children: _tennisBallLevelList.map((level) => Column(
                              children: [
                                InkWell(
                                  onTap: ()=> setState(()=> _selectedTennisBallPlayerLevel = level),
                                  child: Padding(
                                    padding: const EdgeInsets.symmetric(vertical: 4.0),
                                    child: Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        ...
                                        if(_selectedTennisBallPlayerLevel == level)
                                          const Icon(Icons.done,)
                                      ],
                                    ),
                                  ),
                                ),
                                if(level != _tennisBallLevelList[_tennisBallLevelList.length-1])
                                  const Divider(),
                              ],
                            )).toList(),
                          ),
                    ...
                    const SizedBox(height: 20,),
                    MatchGendertypeWidget(selectedType: selectedMatchType, typeList: matchTypeList, onTap: (int index)=> setState(()=> selectedMatchType = matchTypeList[index]), headingTitle: 'Match Type', comingFromFilter: true,),
                    const SizedBox(height: 20,),
                    MatchGendertypeWidget(selectedType: selectedGenderType, typeList: genderList, onTap: (int index)=> setState(()=> selectedGenderType = genderList[index]), headingTitle: 'Gender', comingFromFilter: true,),
                    const SizedBox(height: 40,),
                  ],
                ),
              ),
  ...
  void _initUserInfo()async {
    ...
    try{
      ...
      DocumentSnapshot documentSnapshot = await FirebaseFirestore.instance.collection(userCollection).doc(currentUID).get();
      Map<String, dynamic> userMap = documentSnapshot.data()!as Map<String, dynamic>;
