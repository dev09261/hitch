...
import 'connect_dupr_dialog.dart';
...
class _UserProfileState extends State<UserProfile> {
  ...
  void initState() {
    ...
    _matchTypeList = ['Both', 'Doubles', 'Singles'];
    _genderList = ['Both', 'Males', 'Females'];
    _initUserInfo();
  }
            ...
            children: [
              _loadingUserInfo
                  ...
                  : _isErrorOccurred
                      ? const Center(
                          child: Text(
                            "Failed to get user information, Try again !",
                      ...
                      : SingleChildScrollView(
                          child: Column(
                            ...
                            children: [
                              ...
                              Center(
                                  child: PlayerHitchesCountWidget(
                              ...
                              Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      "Recent video and photos",
                                      style: AppTextStyles.textFieldHeadingStyle
                                          .copyWith(
                                    ...
                                    _userUploadedSportsVideos.isEmpty
                                        ? PickSportVideosPhotosWidget(
                                            onPickSportPhotosAndVideos:
                                                _onPickSportPhotosAndVideos)
                                        : PickedSportVideosPhotosWidget(
                                            pickedFiles:
                                                _userUploadedSportsVideos,
                                            onPickSportVideos:
                                                _onPickSportPhotosAndVideos,
                                  ...
                                  children: [
                                    if (playerTypePickleBal && dupr == null)
                                      PlayerLevelDropdownTextfieldWidget(
                                        ...
                                        hintText:
                                            'Select Pickleball level (DUPR)',
                                      ),
                                    if (playerTypeTennis)
                                      Padding(
                                        padding: EdgeInsets.only(
                                          top: playerTypePickleBal ? 30.0 : 0,
                                        ),
                                        child:
                                            PlayerLevelDropdownTextfieldWidget(
                                          isEmpty:
                                          ...
                                          CoachExperienceLevelsDropDownWidget(
                                              ...
                                              onChanged: (val) => setState(() =>
                                                  _selectedCoachPickleBallExperienceModel =
                                                      val),
                                              experienceLevels:
                                                  _coachPickleBallExperienceList,
                                              width: size.width * 0.8,
                                              hintText:
                                                  'Select years of coaching (DUPR)'),
                                          const SizedBox(
                                            height: 20,
                                          ),
                                          CoachExperienceLevelsDropDownWidget(
                              ...
                              Padding(
                                ...
                                child: AvailabilitySwitch(
                                    onEveryDayChange: (val) =>
                                        setState(() => _isAvailableDaily = val),
                                    onMorningChange: (val) => setState(
                                        () => _isAvailableInMorning = val),
                                    isAvailableDay: _isAvailableDaily,
                                    isAvailableInMorning:
                                        _isAvailableInMorning),
                              ),
                              Padding(
                                padding: const EdgeInsets.symmetric(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    MatchGendertypeWidget(
                                        selectedType: _selectedMatchType,
                                        typeList: _matchTypeList,
                                        onTap: (index) => setState(() =>
                                            _selectedMatchType =
                                                _matchTypeList[index]),
                                        headingTitle: 'Match Type'),
                                    const SizedBox(
                                      height: 20,
                                    ),
                                    MatchGendertypeWidget(
                                        selectedType: _selectedGenderType,
                                        typeList: _genderList,
                                        onTap: (index) => setState(() =>
                                            _selectedGenderType =
                                                _genderList[index]),
                                        headingTitle: 'Gender'),
                                        ...
                                        child: Column(
                                          children: [
                                            _buildTextButton(
                                                ...
                                                onTap: () => InAppPurchaseConfig
                                                    .onRestorePurchaseTap(
                                                        context: context)),
                                            const Divider(),
                                            _buildTextButton(
                                                text: 'Logout',
  ...
  void _onPickProfileImageTap() async {
    ...
    if (imageFile != null) {
      ...
      setState(() {});
      String? url = await userAuthService.getProfileUrl(
          profilePicPath: updatedImagePath!);
  ...
  void _onPickSportPhotosAndVideos() async {
    ...
    if (selectedMedia.isNotEmpty) {
      ...
      List<String> updated =
          selectedMedia.map((uploaded) => uploaded.path).toList();
  ...
  Future<bool> checkForChanges() async {
    ...
    try {
      if (updatedImagePath != null) {
        String? updatedUrl = await userAuthService.getProfileUrl(
            profilePicPath: updatedImagePath!);
        if (updatedUrl != null) {
          imageUrl = updatedUrl;
        }
      ...
      for (var newlyAdded in _newlyUploadedFiles) {
        ...
        if (url != null) {
          newlyUploadedList.add(UploadedFileModel(
            fileName: newlyAdded.name,
            url: url,
          ));
        }
      }
      ...
      List<String> updatedDays = daysAvailable
          .where((dayAvailable) => dayAvailable['isSelected'])
          .toList()
          .map((selectedDay) => selectedDay['day'].toString())
          .toList();
  ...
  void _initUserInfo() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        _contactNumberController =
            TextEditingController(text: user!.cellNumber);
        ...
        _userUploadedSportsVideos =
            user!.uploadedSportsPhotos.map((uploaded) => uploaded.url).toList();
