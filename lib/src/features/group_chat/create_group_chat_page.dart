...
class _CreateGroupChatPageState extends State<CreateGroupChatPage> {
  ...
  bool get _isReadyToContinue => _userHitches.where((userHitch)=> userHitch['isSelected']).toList().isNotEmpty;
  @override
  void initState() {
    _initUserHitches();
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Join the chat", style: AppTextStyles.pageHeadingStyle,),
            ...
            children: [
              Column(
                children: [
                  const Align(
                      alignment: Alignment.center,
                      child: Text("Add or remove Hitches",textAlign: TextAlign.center, style: AppTextStyles.regularTextStyle,)),
                  const SizedBox(height: 20,),
                  Expanded(child: _isLoadingUserHitches
                      ? const LoadingWidget()
                      : _hasError
                      ? Center(child: Text(_errorMessage, textAlign: TextAlign.center, style: AppTextStyles.regularTextStyle,),)
                      : _userHitches.isEmpty
                      ? const Center(child: Text("You need more Hitches for a group chats. \n\nSend “Let’s Play” requests to build your Hitches.",textAlign: TextAlign.center, style: TextStyle(fontSize: 21, color: Colors.black)),)
                      : ListView.builder(
                      itemCount: _userHitches.length,
                      itemBuilder: (ctx, index){
                        HitchesModel hitch = _userHitches[index]['hitch'];
                        bool isSelected = _userHitches[index]['isSelected'];
                        return Column(
                          children: [
                            Padding(
                              ...
                              child: ListTile(
                                ...
                                leading: SizedBox(
                                    width: 45,
                                    child: HitchProfileImage(profileUrl: hitch.user.profilePicture, size: 45)),
                                title: Text(hitch.user.userName, style: AppTextStyles.regularTextStyle.copyWith(fontWeight: FontWeight.w500, color: Colors.black),),
                                trailing: SizedBox(
                                    width: 50,
                                    child: HitchCheckbox(text: '', onChange: (val){
                                      setState(() {
                                        _userHitches[index]['isSelected'] = !isSelected;
                                      });
                                    }, value: isSelected)),
                              ),
                            ),
               ...
               Positioned(
                 ...
                 child: SizedBox(
                 width: 150,
                 child: _isReadyToContinue ? PrimaryBtn(
                     btnText: "Done", onTap: _onCreateChatTap, isLoading: _creatingGroupChat,) : SecondaryBtn(btnText: "Done", onTap: (){
                   Utils.showCopyToastMessage(message: 'Please add hitch to group chat');
                 }),
               ),
              )
            ],
  ...
  void _onCreateChatTap()async{
    ...
    try{
      await ChatService.createGroupChat(hitches: hitches);
      Navigator.of(context).pop();
    }catch(e){
